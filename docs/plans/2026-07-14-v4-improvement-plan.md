...
8. （2026-07-15）粉丝口径：抖音 4.7 万 + 小红书 1.9 万，公众号等其它平台**略过不展示数字**。
9. （2026-07-15）部署选 **c：CF Pages 接 GitHub 自动构建**（仓库已迁 ASI-Mark/mark-portfolio，旧地址转发）。注意：direct-upload 建的 Pages 项目不能转 Git 接入，需**新建**项目再把 asi-mark.cn 域名挪过去。
10. （2026-07-15）马克提出 Hero「开眼」灵感 → 立项为 2.7，设计已出，待马克看方案。
...
- [x] 0.10 `npm run test` 全过 + `next build` 出新 out/ —— 60 个测试全过；`npx next build` 成功（Next.js 16.2.3 + Turbopack，编译 1.8s + TypeScript 检查 1.6s + 静态生成 4/4 页面全部 ○ Static）；out/ 16M，out/cities 41 张照片与预期一致；grep out/ 全目录确认零 mediapipe/globe/手势组件残留 chunk
...
- [ ] 2.7 Hero「开眼」升级（马克 2026-07-15 灵感，设计转译版）：
  - 未擦开态 =「纸背落日」：米白纸面后一层极淡暖金→暗红径向光晕，20-30s 极慢呼吸（宣纸迎着落日的透光感）；叠加「记忆底纹」——马克真实笔记/flomo 词句以 2-4% 对比度烫印在纸面，平时几乎不可见（"信息大爆炸"的纸墨转译：不是满屏飞字，是这张纸本来就写满了字）
  - 手电筒扫过：光圈内底纹词句短暂变得可读一瞬（知识的底层一闪而过）；hero 大字改为**逐字点亮**——每个字单独判定被照到，照到即保持亮
  - 全部字点亮 →「开眼」时刻：光晕满屏漫开一拍、底纹整体微亮一瞬即隐、大字字重由细变正定形（呼应 2.6 可变字体）
  - 反面约束：不用粒子/镜头光斑/闪烁，"天堂感"来自过曝留白+暖光而非特效；移动端判定阈值放宽（拖过即亮）
  - 工作量：中（现有 Canvas 手电筒基建可复用，新增底纹层+逐字命中判定+开眼时刻编排）

## Phase 3 · 沉淀

- [x] 3.1 `马克的Idea/` 建项目档案（2026-07-14 已建）
- [x] 3.2 三份调研报告归档知识库（`02-AI整理/调研存档/2026-07个人网站改版调研/` 三份，2026-07-15）
- [ ] 3.3 改版完成后写复盘，反哺档案
...
三份报告原文已归档：`Obsidian/02-AI整理/调研存档/2026-07个人网站改版调研/`
