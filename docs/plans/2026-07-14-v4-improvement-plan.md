...
- [x] 1.1 新增「造物」板块 ✅2026-07-18 上线（Creations.tsx + creations.json，10 条含 OpenClaw 退役条目；数字标「核对中」待马克逐条核）；同日 **Hero v4「火种照心」移植主站完成**（HeroKaiyan+ActGate 分幕门，旧 Hero 退役，Nav=初心/作品/造物/…）。形态=手记流而非卡片墙（X 调研结论：缩略图卡片墙是 Lovable/v0 模板站的默认脸，最容易撞脸；swyx.io/ideas 式的按日期倒序"一行一物"最有品且贴读书笔记气质）。每行=日期 + 状态点（🟢在跑 🟡搁置 ⚫退役）+ 名字 + 一句人话「它替我干什么」+ 一个真实数字，可展开看两三句细节和截图。候选：coach-v2、选题工作台、coachboard、money-coach、boss-tracker、灵犀、口播白板、记忆系统（qdrant+graphiti）、OpenClaw（⚫退役，写清为什么退役——止损也是能力）
...
- [ ] 2.7 Hero「开眼」升级（马克 2026-07-15 灵感，设计转译版）：
  ...
  - **v4 方向定稿（2026-07-17 马克拍板，推翻链条方案）**：黑曜石黑心（裂缝透红光=还活着）+ 血红双翼 + **五只黑手攫心**（贪=抓取/嗔=死攥/痴=蒙眼/慢=顶上按掌/疑=欲触不触，人的劣根性不着一字）；光=**火种**（普罗米修斯，AI=借来的火）；交互=**刮刮卡治愈**（火种擦到哪永久治愈哪：心变红、翅变白、被照的手松开化烟退散）；原句 3D 悬浮，治愈完成后 morph 成「你想要怎样活这一生？」；终幕振翅一拍→字变→眨眼→第二幕。素材=codex 出图（2 构图×2 状态：桌面横版+手机竖版），出图规矩=全局记忆 feedback-image-gen-use-gpt-not-bailian。底纹小字/暗纱/跳过/回访/⚙面板/reduced-motion 全保留。
  - **v3 方向变更（2026-07-15 马克看过 v2 后拍板）**：①心脏弃水墨改**超写实**（bl 生成 heart-real.png，视觉冲击优先）②链条重做：20 字按读序沿路径缠绕、3D 字牌+相扣链环、断链按读序逐字瓦解③手电筒改**俯视持握**（光标=手持手电，光池+全页极淡暗纱）④加血管纹理层（vessels.png，光照显形）⑤终幕**眨眼转场**进第二幕（造物）——**全站改分幕叙事：第一幕必须走完才见项目**（加 6s 后浮现的「跳过」+localStorage 记忆，防赶时间访客流失）⑥立意锚点：注意力稀缺，心脏只有一个，心中那个目标才是人生的意义。demo：`demos/hero-kaiyan/v3.html`
  - **v2 方向变更（2026-07-15 马克看过 v1 demo 后拍板）**：心脏从"纸背影子"改为**主角**——巨大水墨图谱风心脏居中缓慢跳动；「人不能活一辈子……严父，或者慈母。」逐字装进链环，两条字链立体缠绕心脏（剧本即枷锁）；手电筒回到底部、光锥向上；光照心脏充能→心跳渐强→链环逐个绷断（字坠落化墨）→全断心脏大搏漏拍→「你想要怎样活这一生？」浮现。保留 v1 的底纹小字+光圈显形（马克明确喜欢）。demo：`demos/hero-kaiyan/v2.html`（v1 留档对比）
